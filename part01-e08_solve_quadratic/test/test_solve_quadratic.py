...
class SolveQuadratic(unittest.TestCase):
    ...
    def test_first(self):
        ...
        self.assertIsInstance(
            sol, tuple, f"solve_quadratic should return a tuple. Got {type(sol)}")
        eq = equation(a, b, c)
        self.assertEqual(
            len(sol), 2, msg="Expected two solutions for equation %s!" % eq)
        for x in sol:
            self.assertAlmostEqual(a*x**2 + b*x + c, 0, places=4,
                                   msg="%f is not a solution for equation %s!" % (x, eq))
